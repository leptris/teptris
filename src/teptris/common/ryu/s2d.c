...
enum teptris_ryu_status teptris_ryu_s2d_n(const char * buffer, const int len, double * result) {
  if (len == 0) {
    return TEPTRIS_RYU_INPUT_TOO_SHORT;
  }
  int m10digits = 0;
  int e10digits = 0;
  int dotIndex = len;
  ...
  for (; i < len; i++) {
    ...
    if (c == '.') {
      if (dotIndex != len) {
        return TEPTRIS_RYU_MALFORMED_INPUT;
      }
      dotIndex = i;
      continue;
    }
    ...
    if (m10digits >= 17) {
      return TEPTRIS_RYU_INPUT_TOO_LONG;
    }
    m10 = 10 * m10 + (c - '0');
    if (m10 != 0) {
      m10digits++;
